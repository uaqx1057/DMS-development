                    ...
                    <div class="col-lg-4">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="text-muted">{{ $data['totals'][$period] ?? 0 }}</p>
                                <div class="flex-wrap d-flex justify-content-evenly">
                                    <p class="mb-0 text-muted">
                                       Delivered: <i class="align-middle mdi mdi-numeric-{{ $data['statusCounts']['Drop'][$period] ?? 0 }}-circle text-success fs-18 me-2"></i>
                                    </p>
                                    <p class="mb-0 text-muted">
                                        Pickup: <i class="align-middle mdi mdi-numeric-{{ $data['statusCounts']['Pickup'][$period] ?? 0 }}-circle text-info fs-18 me-2"></i>
                                    </p>
                                    <p class="mb-0 text-muted">
                                        Cancel: <i class="align-middle mdi mdi-numeric-{{ $data['statusCounts']['Cancel'][$period] ?? 0 }}-circle text-danger fs-18 me-2"></i>
                                    </p>
                                </div>
                            </div>
                            <div class="progress animated-progress rounded-bottom rounded-0" style="height: 6px;">
                                @php
                                    ...
                                    $dropPercentage = round((($data['statusCounts']['Drop'][$period] ?? 0) / $total) * 100);
                                    $pickupPercentage = round((($data['statusCounts']['Pickup'][$period] ?? 0) / $total) * 100);
                                    $cancelPercentage = round((($data['statusCounts']['Cancel'][$period] ?? 0) / $total) * 100);
                                @endphp
