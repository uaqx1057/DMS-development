    ...
    public function all()
    {
        ...
        ->with([
            ...
            'coordinator_report' => function ($query) {
                $query->where('status', CoordinatorReportStatus::Approved)->with('report_fields');
            }
        ])
        ->get()
    ...
    public function getBusinesses()
{
    ...
    $businesses = Business::with('coordinator_report.report_fields')->get();

    // Get the field ID for "Total Orders" once to optimize the loop
    $fieldId = Field::where('name', 'Total Orders')->pluck('id')->first();

    foreach ($businesses as $business) {
        $totalSum = 0;
        // Calculate total orders for each business based on approved reports
        foreach ($business->coordinator_report as $report) {
            if ($report->status == CoordinatorReportStatus::Approved->value) {
                $totalSum += $report->report_fields->where('business_id', $business->id)->where('field_id', $fieldId)->sum('value');
            }
        }

        // Attach total_orders attribute to the business instance
...
public function getRevenueStats()
{
    ...
    foreach ($drivers as $driver) {
        ...
        foreach ($driver->coordinator_report as $report) {
            // Sum "Total Orders" for the report, filtering by field_id
            $reportSum = $report->report_fields
                ->where('field_id', $fieldId)
                ->where('value', '!=', '')
                ->sum('value');
            $totalSum += $reportSum;

            // Aggregate total report_fields by business_id for each coordinator report
            foreach ($report->report_fields->where('value', '!=', '') as $field) {
        ...
        // Store calculated total orders and business reports on the driver
        $driver->total_orders = $totalSum;
        $driver->business_reports = array_values($business_reports);

        // Calculate total revenue per business
            ...
            if ($businessCalculations && $businessCalculations->driver_calculations) {
                foreach ($businessCalculations->driver_calculations as $calculation) {
                    if ($calculation->calculation_type == 'RANGE' &&
                        ...
                        // Apply calculation for RANGE type and break out once matched
                        $total_revenue += $business_report['total_orders'] * $calculation->amount;
                        break;
                    }
                    if ($calculation->calculation_type == 'FIXED') {
                        // Apply calculation for FIXED type without breaking
                        $total_revenue += $business_report['total_orders'] * $calculation->amount;
                    }
                }
            }
        ...
        // Calculate total cost based on driver details
        ...
        // Sum up the total costs for the driver
